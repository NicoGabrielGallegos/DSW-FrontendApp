...
export default function Docentes() {
    ...
    useEffect(() => {
        async function initPage() {
            ...
            if (docenteParam) {
                setSelectedMateria(docenteParam)
            }
    ...
    useEffect(() => {
        async function reloadMaterias() {
            await fetchMaterias(selectedMateria)
        }
        reloadMaterias()
    }, [selectedMateria])
        ...
        content = (
            <Grid container spacing={2}>
                {materias.map((materia, idx) => {
                    return (
                        <Grid size={12} key={idx}>
                            <MateriaCard id={materia._id} nombreMateria={materia.descripcion} options={{ docente: selectedMateria }} />
                        </Grid>
                    )
                })}
            </Grid>
