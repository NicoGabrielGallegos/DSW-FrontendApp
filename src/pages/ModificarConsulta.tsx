...
export default function ModificarConsulta() {
    // Hook para recibir la consulta a modificar
    const location = useLocation()
    // Colecciones de materias, docentes y consultas
    const [materias, setMaterias] = useState<Materia[]>([])
    // Variables para el selector de materia
    const [valueMateria, setValueMateria] = useState<{ id: string, label: string } | null>(location.state && {id: location.state.dictado.materia._id, label: location.state.dictado.materia.descripcion} || null)
    const [inputMateria, setInputMateria] = useState<string>("")
    // Variables para los selectores de fecha y hora
    const [valueFecha, setValueFecha] = useState<Date | null>(location.state && new Date(location.state.horaInicio) || null)
    const [valueHoraInicio, setValueHoraInicio] = useState<Date | null>(location.state && new Date(location.state.horaInicio) || null)
    const [valueHoraFin, setValueHoraFin] = useState<Date | null>(location.state && new Date(location.state.horaFin) || null)
    const hoy = new Date()
    ...
    const handleVolver = () => navigate(ROUTES.CONSULTA_BY_ID(location.state._id))

    const handleRealizarCambios = async () => {
        ...
        try {
            await apiClient.put(API_ROUTES.CONSULTAS.UPDATE(location.state._id), { headers: { "Authorization": `Bearer ${localStorage.getItem("token")}` }, body })
            handleVolver()
        } catch (err: any) {
            setAlert({ message: err.message, severity: "error" })
        }
    ...
    if (error) {
        ...
        content = (
            <Paper sx={{ p: 4, mt: 2, maxWidth: 500 }} elevation={3}>
                <Typography variant="h5">Ingrese los nuevos datos de la consulta</Typography>
                <Grid container spacing={2} mt={2}>
                    <Grid size={12}>
                        <TextField id="consulta" label="Consulta" size="small" variant="standard" fullWidth disabled value={location.state._id} onChange={() => {}}></TextField>
                    </Grid>
                    <Grid size={12}>
                ...
                <Grid container size={12} justifyContent={"end"} spacing={2}>
                    <Button variant="outlined" sx={{ mt: 3 }} onClick={handleVolver}>Cancelar</Button>
                    <Button variant="contained" sx={{ mt: 3 }} onClick={handleRealizarCambios}>Realizar cambios</Button>
                </Grid>
            </Paper>
        )
    }
